...
export const TutorialDisplay: React.FC<TutorialDisplayProps> = ({ tutorial, onUpdateStepImage, onExport }) => {
    return (
        <div className="w-full max-w-5xl mx-auto">
            ...
            <div id="pdf-content" className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl p-6 md:p-12">
                <header className="text-center border-b-2 border-gray-200 dark:border-gray-700 pb-8 mb-10">
                    <h1 className="text-4xl md:text-5xl font-extrabold text-gray-900 dark:text-white">{tutorial.equipment.name}</h1>
                    <p className="text-xl text-gray-600 dark:text-gray-400 mt-2">{tutorial.equipment.model}</p>
                    <p className="text-lg text-blue-600 dark:text-blue-400 font-semibold mt-1">{tutorial.equipment.application}</p>
                </header>
                ...
                <Section title="Passos de Instalação">
                    <div className="space-y-12">
                        {tutorial.installationSteps.map((step) => (
                            <StepItem key={step.id} step={step} onUpdateImage={onUpdateStepImage} />
                        ))}
                    </div>
                </Section>
                ...
                <Section title={tutorial.testingProcedures.title}>
